...
public class Holder : MonoBehaviour
{
    ...
    public void AddLiquid(int groupId, float value = 0)
    {
        ...
        if (liquid.IsBottomLiquid)
            liquid.transform.position = topPoint;
        else
            liquid.transform.position = GetTopNewPoint();
    ...
    public Vector2 GetTopNewPoint()
    {
        // Liquid sprite = _liquids.Last().Renderer.GetComponent<Liquid>();
        Liquid sprite = _liquids.Last().GetComponent<Liquid>();
        
        Debug.Log("Pos " + sprite.TopPosition.position);
        return sprite.TopPosition.position;
        /*Debug.Log("SIZE " + sprite.bounds.size.y);
        return transform.position + new Vector3(0, sprite.bounds.size.y, 0);*/
        
        // return sprite.transform.TransformPoint(sprite.bounds.max.y * Vector2.up);

        /*if (_liquids.Any())
        {
            // Получаем верхнюю точку последнего добавленного напитка
            return _liquids.Last().GetTopPoint();
        }
        else
        {
            // Если напитков нет, возвращаем начальную точку
